...
from download_interface import DownloadWindow
from download_interface import check_models
from tts_setting import TTS_Setting
...
class Win(QOpenGLWidget):
    ...
    def init_system_tray(self):
        ...
        self.tts_setting_action = QAction("TTS设置", self)
        ...
        self.tray_menu.addAction(self.tts_setting_action)
        ...
        self.tts_setting_action.triggered.connect(self.open_tts_settings)
        # 设置托盘图标的上下文菜单
    ...
    def open_tts_settings(self):
        self.tts_settings = TTS_Setting()
        self.tts_settings.show()
    ...
    def cleanup(self):
        ...
        for worker in self.active_tts_workers:
            if isinstance(worker, TTSWorker):
                worker.stop()
        self.thread_pool.clear()  # 取消未开始的任务
    ...
    def initializeGL(self):
        ...
        # self.model.SetRandomExpression(True)
        # 初始加载模型路径
    ...
    def handle_wakeup(self, is_detected: bool):
        ...
        if is_detected:
            ...
            self.talking_settings.text_win.show()
...
if __name__ == "__main__":
    ...
    # 检查缺失的模型
    missing_models = check_models()  # 返回模型列表而非布尔值
    ...
    if missing_models:
        ...
        # 遍历所有缺失模型并启动下载
        for model_id, save_path in missing_models:
            Path(save_path).mkdir(parents=True, exist_ok=True)
            download_window.start_download(model_id, save_path)
            # 确保下载完成信号连接
            download_window.worker.finished.connect(on_download_finished)
